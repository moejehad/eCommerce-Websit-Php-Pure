...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    if(empty($address)){
      ...
      if(empty($formErros)){
            ...
            if ($rows['balance'] >= $theSum){
                
            $balance = $rows['balance'] - $theSum;
            ...
            $successMsg = 'تم إرسال طلبك وسيتم التواصل معك في أقرب وقت';
                ...
                $error = 'رصيدك غير كافي لعملية الشراء ';
            }
...
<div class="container">
    ...
    if(isset($successMsg)){
        ...
        <form class="cart" action="walletCart.php?itemid=<?php echo $item['item_ID'] ?>&name=<?php echo $item['Name'] ?>" method="POST">
            ...
            <input type="hidden" name="status" value="مدفوع" readonly required />
